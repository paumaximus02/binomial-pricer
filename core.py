...
def price_european(
    ...
    _, u, d, p, discount = _crr_params(spot, strike, rate, vol, time_to_expiry, steps)
...
def price_option(
    ...
    if is_american:
        return price_american(spot, strike, rate, vol, time_to_expiry, steps, is_call)
    return price_european(spot, strike, rate, vol, time_to_expiry, steps, is_call)
...
def compute_greeks(
    ...
    """
    price = price_option(spot, strike, rate, vol, time_to_expiry, steps, is_call, is_american)

    d_spot = max(spot * 0.01, 0.01)
    p_up = price_option(spot + d_spot, strike, rate, vol, time_to_expiry, steps, is_call, is_american)
    p_down = price_option(spot - d_spot, strike, rate, vol, time_to_expiry, steps, is_call, is_american)
    ...
    if d_t > 0:
        p_shorter = price_option(spot, strike, rate, vol, time_to_expiry - d_t, steps, is_call, is_american)
        theta = (p_shorter - price) / d_t
    else:
        theta = 0.0

    d_vol = 0.01
    p_vol_up = price_option(spot, strike, rate, vol + d_vol, time_to_expiry, steps, is_call, is_american)
...
def implied_volatility(
    ...
    lo, hi = 1e-4, 5.0
    f_lo = price_option(spot, strike, rate, lo, time_to_expiry, steps, is_call, is_american) - market_price
    f_hi = price_option(spot, strike, rate, hi, time_to_expiry, steps, is_call, is_american) - market_price
    ...
    for _ in range(max_iter):
        mid = 0.5 * (lo + hi)
        f_mid = price_option(spot, strike, rate, mid, time_to_expiry, steps, is_call, is_american) - market_price
        if abs(f_mid) < tol:
            return mid
        if f_lo * f_mid <= 0:
            hi, f_hi = mid, f_mid
